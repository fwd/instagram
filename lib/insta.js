    ...
    scrapeInput(toJson){
        return new Promise( async (resolve, reject) => {
            ...
            try{
                ...
                // profile information
                let profile = toJson.entry_data[fromInput][0].graphql[this._scrapeType]
                ...
                if (profile.edge_followed_by) {
                    this.profile.followers = profile.edge_followed_by.count
                    this.profile.following = profile.edge_follow.count
                }   
                
            } catch(error){
                return reject({ message: "Can't parse json", error: error.message, json: toJson });
            }
        ...
        return new Promise( async (resolve, reject) => {
            for(let i = 0; i < edges.length; i++){
                ...
                item = {
                    ...
                    captionText: item.edge_media_to_caption && item.edge_media_to_caption.edges[0] ? item.edge_media_to_caption.edges[0].node.text : false,
                    ...
                    commentsCount: item.edge_media_to_comment.count,
                    comments: item.edge_media_to_comment.edges ? item.edge_media_to_comment.edges.map(a => a.node) : [],
